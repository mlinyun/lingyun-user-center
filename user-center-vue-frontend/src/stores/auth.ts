...
// 内存态缓存 TTL（仅用于页面驻留期间的体验优化，刷新后自然失效）
...
export const useAuthStore = defineStore("auth", () => {
    ...
    // 最近一次后端校验的时间戳（Unix ms，仅用于本次页面驻留周期）
    const lastValidatedAt = ref<number | null>(null);
    // 内存态缓存到期时间戳（Unix ms），不作为安全判断依据
    ...
    /**
     * 判断内存态缓存是否仍然有效。过期后会在 ensureCacheFreshness 中重置状态回 unknown。
     */
     ...
     * 短路策略：
     * - 普通模式：内存态缓存有效时跳过后端请求
     * - 严格模式：受保护路由使用，每次都请求后端确保 Session 一致性
     * - force=true：无条件请求后端
     */
